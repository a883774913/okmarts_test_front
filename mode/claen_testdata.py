...
class Clean_testdata:
    ...
    def claen_address(self, number):
        ...
        for i in range(1, number):
            try:
                Address_mode().address_mode(Parameters={'casename': '输入正确参数删除地址成功', 'url': 'http://18.118.13.94:81/jeecg-boot/sys/api/homePage/updateAddress',
                                                        'data': '{"type":3,"id":"1506562052406251520"}', 'content_type': 'application/json', 'result': '成功',
                                                        'assert_way': 'message', 'mode': '删除地址'})
