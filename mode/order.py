...
class order:
    ...
    def order(self,Parameters):
        ...
        print(f'shopcar_info为{shopcar_info}')

        if '"addressId":' in data:
            data = Parameters_cls().addressId(casename, data,spec_case['spec_addressid'])
        if '"count":' in data:
            data = Parameters_cls().count(casename, data, spec_case['spec_count'], shopcar_info,mode)
        if '"shopCartId":' in data:
            data = Parameters_cls().shopCartId(casename, data, spec_case['spec_shopcartid'], shopcar_info)
        if '"couponsid":' in data:
            data = Parameters_cls().couponsid(casename, data, spec_case['Spec_couponsid'], shopcar_info)
